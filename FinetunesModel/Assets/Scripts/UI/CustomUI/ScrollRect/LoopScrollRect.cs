...
namespace UnityEngine.UI
{
	...
	public class LoopScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
	{
        ...
        [SerializeField]
        private bool m_EnableDataLoop;
        public bool EnableDataLoop { get { return m_EnableDataLoop; } set { m_EnableDataLoop = value; } }

		[SerializeField]
		private bool m_EnableFixedVelocity;
		public bool EnableFixedVelocity { get { return m_EnableFixedVelocity; } set { m_EnableFixedVelocity = value; } }

		[SerializeField]
		private float m_FixedVelocity;
		public float FixedVelocity { get { return m_FixedVelocity; } set { m_FixedVelocity = value; } }

        protected bool canMovement = false;
		...
		private Vector3 m_realContentStartDelta;
		private Vector3 m_realContentEndDelta;
		private Bounds m_RealContentBounds;
		...
		private Bounds m_PrevRealContentBounds;
		...
		protected void UpdateRealContentBoundsInfo(Vector3 startDelta, Vector3 endDelta)
		{
			m_realContentStartDelta = startDelta;
			m_realContentEndDelta = endDelta;
		}

		protected void ForceSetContentPosition(Vector2 newPosition)
		{
			Vector2 delta = newPosition - m_Content.anchoredPosition;
		...
		protected virtual void LateUpdate()
		{
			...
			if (!m_Dragging && (offset != Vector2.zero || m_Velocity != Vector2.zero))
			{
				...
				for (int axis = 0; axis < 2; axis++)
				{
					...
					if (canMovement && m_MovementType == MovementType.Elastic && offset[axis] != 0)
					{
			...
			if (m_Dragging && m_Inertia)
			{
				...
				if (m_EnableFixedVelocity) {
					if (m_Horizontal) {
						m_Velocity [0] = newVelocity [0] > 0 ? Mathf.Min (m_FixedVelocity, newVelocity [0]) :
							Mathf.Max (-m_FixedVelocity, newVelocity [0]);
					}
					if (m_Vertical) {
						m_Velocity [1] = newVelocity [1] > 0 ? Mathf.Min (m_FixedVelocity, newVelocity [1]) :
							Mathf.Max (-m_FixedVelocity, newVelocity [1]);
					}
				}
			}
			...
			if (m_ViewBounds != m_PrevViewBounds ||
				...
				m_RealContentBounds != m_PrevRealContentBounds)
			{
		...
		private void UpdatePrevData()
		{
			...
			m_PrevRealContentBounds = m_RealContentBounds;
		}
		...
		private void UpdateScrollbars(Vector2 offset)
		{
			if (m_HorizontalScrollbar)
			{
				if (m_RealContentBounds.size.x > 0)
					m_HorizontalScrollbar.size = Mathf.Clamp01((m_ViewBounds.size.x - Mathf.Abs(offset.x)) / m_RealContentBounds.size.x);
				else
			...
			if (m_VerticalScrollbar)
			{
				if (m_RealContentBounds.size.y > 0)
					m_VerticalScrollbar.size = Mathf.Clamp01((m_ViewBounds.size.y - Mathf.Abs(offset.y)) / m_RealContentBounds.size.y);
				else
		...
		public float horizontalNormalizedPosition
		{
			...
			{
				UpdateBounds();
                if (m_RealContentBounds.size.x <= m_ViewBounds.size.x)
                    return (m_ViewBounds.min.x > m_RealContentBounds.min.x) ? 1 : 0;
                return (m_ViewBounds.min.x - m_RealContentBounds.min.x) / (m_RealContentBounds.size.x - m_ViewBounds.size.x);
			}
			set
			{
		...
		public float verticalNormalizedPosition
		{
			...
			{
				UpdateBounds();
				if (m_RealContentBounds.size.y <= m_ViewBounds.size.y)
					return (m_ViewBounds.min.y > m_RealContentBounds.min.y) ? 1 : 0;
				return (m_ViewBounds.min.y - m_RealContentBounds.min.y) / (m_RealContentBounds.size.y - m_ViewBounds.size.y);
			}
			set
			{
		...
		protected virtual void SetLoopNormalPosition(float newPosition) { }
		...
		private void SetNormalizedPosition(float value, int axis)
		{
			...
			float hiddenLength = m_RealContentBounds.size[axis] - m_ViewBounds.size[axis];
			// Where the position of the lower left corner of the content bounds should be, in the space of the view.
			float contentBoundsMinPosition = m_ViewBounds.min[axis] - value * hiddenLength;
			// The new content localPosition, in the space of the view.
			float newLocalPosition = m_Content.localPosition[axis] + contentBoundsMinPosition - m_RealContentBounds.min[axis];
			...
			if (Mathf.Abs (localPosition [axis] - newLocalPosition) > 0.01f)
			{
                if (m_Horizontal)
                {
                    SetLoopNormalPosition (newLocalPosition - m_realContentStartDelta[axis]);
                }
                else
                {
                    SetLoopNormalPosition (newLocalPosition + m_realContentStartDelta[axis]);
		...
		public virtual void SetLayoutHorizontal()
		{
			...
			if (m_HSliderExpand || m_VSliderExpand)
			{
				...
				m_RealContentBounds = GetRealBounds ();
			}
		...
		private void UpdateBounds()
		{
			...
			Vector3 realContentSize = m_RealContentBounds.size;
			Vector3 realContentPos = m_RealContentBounds.center;
			Vector3 realExcess = m_ViewBounds.size - realContentSize;
			if (realExcess.x > 0)
			{
				realContentPos.x -= realExcess.x * (m_Content.pivot.x - 0.5f);
				realContentSize.x = m_ViewBounds.size.x;
			}
			if (realExcess.y > 0)
			{
				realContentPos.y -= realExcess.y * (m_Content.pivot.y - 0.5f);
				realContentSize.y = m_ViewBounds.size.y;
			}

			m_RealContentBounds.size = realContentSize;
			m_RealContentBounds.center = realContentPos;

		}

		private readonly Vector3[] m_Corners = new Vector3[4];
		private readonly Vector3[] m_RealCorners = new Vector3[4];
		...
		private Bounds GetRealBounds()
		{
            ...
            if (m_Horizontal)
            {
                m_RealCorners [0] = m_Corners[0] - m_realContentStartDelta;
                m_RealCorners [1] = m_Corners[1] - m_realContentStartDelta;
                m_RealCorners [2] = m_Corners[2] + m_realContentEndDelta;
                m_RealCorners [3] = m_Corners[3] + m_realContentEndDelta;
            }
            else
            {
                m_RealCorners [0] = m_Corners[0] - m_realContentEndDelta;
                m_RealCorners [1] = m_Corners[1] + m_realContentStartDelta;
                m_RealCorners [2] = m_Corners[2] + m_realContentStartDelta;
                m_RealCorners [3] = m_Corners[3] - m_realContentEndDelta;
            }
			...
			for (int j = 0; j < 4; j++)
			{
				Vector3 v = toLocal.MultiplyPoint3x4(m_RealCorners[j]);
				vMin = Vector3.Min(v, vMin);
				vMax = Vector3.Max(v, vMax);
			}
		...
		private Vector2 CalculateOffset(Vector2 delta)
		{
			...
			Vector2 min = m_RealContentBounds.min;
			Vector2 max = m_RealContentBounds.max;
